...
p_f = PyPtxt(pyfhel=HE_f, fileName=tmp_dir_name + "/p.ctxt")
...
print(f"   - s_context: {s_context[:10].encode()}...")
print(f"   - s_public_key: {s_public_key[:10].encode()}...")
print(f"   - s_secret_key: {s_secret_key[:10].encode()}...")
print(f"   - s_relin_key: {s_relin_key[:10].encode()}...")
print(f"   - s_rotate_key: {s_rotate_key[:10].encode()}...")
print(f"   - s_c: {s_c[:10].encode()}...")
print(f"   - s_p: {s_p[:10].encode()}...")
...
p_b = PyPtxt(pyfhel=HE_b); p_b.from_bytes(s_p) 
...
#      |__  To avoid accidental sharing of the secret key.
