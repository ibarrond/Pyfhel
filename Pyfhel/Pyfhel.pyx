...
cdef class Pyfhel:
    ...
    def __init__(self,
        ...
        Attributes:
            context_params (dict|str|pathlib.Path, optional): dictionary of context
                    parameters to run contextGen(), or alternatively a string with the name of a saved context, to read with load_context().
    ...
    @property
    def sec(self):
        """Security of thecontextparameters (bits)."""
        return (<Afseal*>self.afseal).get_sec()

    @property
    def qi(self):
        """Coefficientmoduli. Chain of primesthat composition."""
        return self.get_qi()
    ...
    cpdef string contextGen(self,
        ...
        *CKKS scheme*: vectorized approximate fixed point operations in SIMD. The
        ...
        if s==Scheme_t.bfv:
            ...
            assert (t_bits>0 or t>0),\
                "BGV scheme requires plain_modulus (`t_bits` or `t`) to be set"
            assert (sec>0) or (not qi_sizes.empty()) or (not qi.empty()),\
                "BGV scheme requires `sec` or `qi` to be set."
            if (sec>0):       # Override `qi_sizes`
                qi_sizes.clear()
            self._scale = 1   # Default scale
    ...
    cpdef size_t save_context(self, fileName, str compr_mode="zstd"):
        ...
        Return:
            size_t:numberof bytessaved/loaded
        """
