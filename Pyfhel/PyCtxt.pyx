...
cdef class PyCtxt:
    ...
    cpdef size_t save(self, str fileName, str compr_mode="zstd"):
        ...
        try:
            size = self._pyfhel.afseal.save_ciphertext(deref(outputter), bcompr_mode, deref(self._ptr_ctxt))
        finally:
            del outputter
        return size
    ...
    cpdef size_t load(self, str fileName, object scheme=None):
        ...
        try:
            size = self._pyfhel.afseal.load_ciphertext(deref(inputter), deref(self._ptr_ctxt))
        finally:
            del inputter
