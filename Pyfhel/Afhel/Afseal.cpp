...
// -----------------------------------------------------------------------------
// CONTEXT GENERATION
void Afseal::ContextGen(scheme_t scheme, uint64_t poly_modulus_degree, uint64_t plain_modulus_bit_size, uint64_t plain_modulus, int sec, std::vector<int> qs) {
...
int Afseal::get_sec() {
  if (this->context==NULL) { throw std::logic_error("<Afseal>: Context not initialized"); }
  return static_cast<std::underlying_type<sec_level_type>::type>(this->context->first_context_data()->qualifiers().sec_level);
}
// GETTERS
...
#pragma omp parallel for
  for (size_t j = 0; j < coeff_modulus.size(); j++) {
    util::add_poly_coeffmod(eval_repr_coeff_iter + (j*coeff_count),
                            dynamic_cast<const AfsealPoly&>(other).eval_repr_coeff_iter + (j*coeff_count),
                            coeff_count,
                            coeff_modulus[j],
...
#pragma omp parallel for
  for (size_t j = 0; j < coeff_modulus.size(); j++) {
    util::dyadic_product_coeffmod(eval_repr_coeff_iter + (j*coeff_count),
                                  (*o_p).eval_repr_coeff_iter + (j*coeff_count),
                                  coeff_count,
                                  coeff_modulus[j],
                                  eval_repr_coeff_iter  //TODO: Check if this is safe (used to be result + ..)
                                      + (j*coeff_count));
