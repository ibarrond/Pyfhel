...
class PyfhelTestCase(unittest.TestCase):
    ...
    def test_Pyfhel_2a_encode_decode_int(self):
        ...
        self.assertEqual(
            self.ptxt.to_string(), b"1x^6 + 1x^5 + 1x^4 + 1x^3 + 1x^2 + 1x^1 + 1"
        )
    ...
    def test_Pyfhel_2d_encode_decode_array(self):
        ...
        self.assertTrue(
            np.alltrue(
                self.pyfhel.decodeArray(self.ptxt)[:6] == np.array([1, 2, 3, 4, 5, 6])
            )
        )
    ...
    def test_Pyfhel_3d_encrypt_decrypt_array(self):
        ...
        self.assertTrue(
            np.alltrue(
                self.pyfhel.decryptArray(self.ctxt)[:6] == np.array([1, 2, 3, 4, 5, 6])
            )
        )
    ...
    def test_Pyfhel_4a_operations_integer(self):
        ...
        self.ctxt_mult2 = self.pyfhel.multiply_plain(
            self.ctxti, self.ptxti, in_new_ctxt=True
        )
    ...
    def test_Pyfhel_4c_operations_batch_array(self):
        ...
        self.assertEqual(
            self.pyfhel.decryptBatch(self.ctxt_add2)[:6], [13, 17, 21, 25, 29, 33]
        )
        self.assertEqual(
            self.pyfhel.decryptBatch(self.ctxt_sub)[:6], [7, 7, 7, 7, 7, 7]
        )
        self.assertEqual(
            self.pyfhel.decryptBatch(self.ctxt_sub2)[:6], [-11, -13, -15, -17, -19, -21]
        )
        self.assertEqual(
            self.pyfhel.decryptBatch(self.ctxt_mult)[:6], [-6, -10, -12, -12, -10, -6]
        )
        self.assertEqual(
            self.pyfhel.decryptBatch(self.ctxt_mult2)[:6], [12, 30, 54, 84, 120, 162]
        )
        self.assertEqual(
            self.pyfhel.decryptBatch(self.ctxt_rotate)[:6], [0, 0, 1, 2, 3, 4]
        )
        self.assertEqual(
            self.pyfhel.decryptBatch(self.ctxt_rotate2)[:6], [3, 4, 5, 6, 0, 0]
        )
